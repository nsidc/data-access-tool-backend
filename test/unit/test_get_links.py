...
def _get_links_response(cursor: str | None = None):
    ...
    with app.test_client() as client:
        ...
        # cmr_query_params.
        cmr_request_params = "provider=NSIDC_CPRD&page_size=5&sort_key[]=-start_date&sort_key[]=producer_granule_id&short_name=ATL06&version=6&version=07&version=007&temporal[]=2018-10-14T00:00:00Z,2025-02-25T00:25:20Z&bounding_box=-180,-90,180,90&options[producer_granule_id][pattern]=true&producer_granule_id[]=*ATL06_2024*_0804*_007_01.h5*"
...
def test_get_links():
    ...
    for idx in range(1, 6 + 1):
        if result.json["done"]:
            # 30 ganules, 5 granules per page.
            raise RuntimeError(f"Expected only 6 pages of results. Done after {idx}")
